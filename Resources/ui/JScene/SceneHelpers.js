    ...
    init : function(scene) {
        ...
        scene.isPointInPoly = function(poly, point, scaleX, scaleY) {
            ...
            for (var i = 0, j = length - 1; i < length; j = i++) {
                ...
                if (
                    ((piy > testY) != (pjy > testY)) &&
                    (testX < (pjx - pix) * (testY - piy) / (pjy - piy) + pix)
                ) {
                    result = !result;
                }
        ...
        scene.enlargeSpriteTouchZone = function(sprite, width, height) {
            if (width  === undefined) width  = 100 * scene.scaleX;
            if (height === undefined) height = 100 * scene.scaleY;
        ...
        scene.linkParts = function(partNames, subsprite) {
            ...
            var result = [];
            var prefix = subsprite ? (
                (
                    (typeof subsprite === "string") ? 
                        subsprite :
                        subsprite.loadedData.name
                ) + "_"
            ) : "";
            ...
            partNames.forEach(function(e) {
                ...
                result.push(result[e] =
                    scene.bodyByName(name) ||
                    scene.spriteByName(name)
                );
            });
        ...
        scene.createParticles = function(params) {
            ...
            _.defaults(params, { x: 0, y: 0, z: 0, alpha: 1 });
            ...
            // if (scene.isRetina) {
                ...
                function rescaleFx() {
                    ...
                    [
                        "sourcePositionVariance_X", "gravity_X",  "startParticleSize", 
                        "sourcePositionVariance_Y", "gravity_Y", "finishParticleSize", 
                         "startParticleSizeVariance", "maxRadius", "maxRadiusVariance",
                        "finishParticleSizeVariance", "minRadius", "radiusSpeed", 
                        "speed", "speedVariance"
                    ].forEach(function(e) {
                        var a = sprite[e];
                        if (a !== 0) {
